"""Unit tests for the PhiEgg Ontologylogy SDK and Agent Platform."""
...
class TestRequestInfoAndStreaming:
    def test_request_info_path_building(self):
        from src.phiegg import RequestInfo
        req = RequestInfo(
            method="POST",
    ...
    async def test_page_iterator(self):
        from src.phiegg import AsyncPageIterator
    ...
    def test_with_raw_and_streaming_response(self):
        from src.phiegg import with_raw_response, with_streaming_response
...
class TestPhiEggClient:
    @pytest.mark.asyncio
    async def test_client_subclients_and_data_resolution(self):
        client = PhiEggClient(auth=TokenAuth("phi_demo_123"))
