...
class ParityAuditor:
    ...
    def audit(self) -> ParityReport:
        ...
        # Check domain agents in src/phiadk/agents
        phiadk_agents_dir = self._repo_root / "src" / "phiadk" / "agents"
        agent_dirs = [d for d in phiadk_agents_dir.iterdir() if d.is_dir() and d.name.startswith("phi") and (d / "schema.json").exists()] if phiadk_agents_dir.exists() else []
