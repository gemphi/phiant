...
class DocIngestionEngine:
    ...
    def transform_markdown(self, text: str, module_name: str) -> str:
        """Transforms SDK client references to PhiEggClient while preserving exact Palantir APIs and names."""
        # 1. Imports and Client Instantiations
        text = re.sub(r"from foundry_sdk import FoundryClient", "from phiegg import PhiEggClient", text)
        text = re.sub(r"import foundry_sdk", "import phiegg", text)
        text = re.sub(
        ...
        text = re.sub(r"foundry_sdk\.", "phiegg.", text)
        text = re.sub(r"FoundryClient", "PhiEggClient", text)
