...
Commands:
    ...
    phi server start              — Start the PhiEgg AIP server
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(prog="phi", description="PhiEgg CLI — Palantir-Style Ontologylogy Platform Toolchain")
    ...
    # ── phi server ───────────────────────────────────────────────────
    srv_parser = subparsers.add_parser("server", help="PhiEgg AIP Platform Server")
...
def main(args: Optional[List[str]] = None) -> int:
    ...
    if opts.command == "agents":
        ...
        src_phiegg = Path(__file__).parent.parent
        count = 0
        for p in src_phiegg.glob("phi*/schema.json"):
            aid = p.parent.name
            agent = client.agents.get(aid)
            if agent and agent.card:
                with open(p, "w", encoding="utf-8") as f:
                    json.dump(agent.card.to_dict(), f, indent=2)
                print(f"  [OK] Updated spec: {p.relative_to(src_phiegg)}")
                count += 1
        ...
        if opts.subcommand == "start":
            ...
            print(f"\n Starting PhiEgg AIP Server on http://{opts.host}:{opts.port} ...\n")
        ...
        if opts.subcommand == "start":
            ...
            print("\n Starting PhiEgg MCP Server (JSON-RPC stdio transport)...", file=sys.stderr)
            asyncio.run(mcp_srv.run_stdio())
            return 0

    elif opts.command == "repl":
        print("\n Starting PhiEgg Interactive REPL...")
