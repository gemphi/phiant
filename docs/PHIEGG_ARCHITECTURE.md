# PhiEgg Architecture & SDK Guide

> **Ecosystem Paradigm**: Repackaged from `phient` into `phiegg` inspired by Palantir's `foundry_sdk`, utilizing **Topology-based AI abstractions** over legacy Ontology jargon, strict **Data-as-Sets separation**, and a **Recursive Fractal Lifecycle** (`envision → apply → eval → iterate/scale`).
...
Traditional enterprise systems (like Palantir Foundry) model domain objects as an *Ontology*. PhiEgg modernizes this into a **Topological Framework** aligned with modern topological deep learning, Persistent Homology, and Simplicial Complex reasoning.

| Palantir Ontology | PhiEgg Topology | Mathematical / Structural Meaning |
|---|---|---|
...
```python
from phiegg import PhiEggClient, TokenAuth, Config
...
PhiEgg exposes enterprise-grade `P*` standard classes that provide 1:1 parity with Palantir Foundry SDK conventions:
